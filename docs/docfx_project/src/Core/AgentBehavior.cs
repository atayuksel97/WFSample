...
namespace NoOpArmy.WiseFeline
{
    ...
    public class AgentBehavior : ScriptableObject
    {
        ...
        internal void AddRuntimeActionSet(ActionSet set, Brain brain)
        {
            ...
            {
                if (aSet.guid == set.guid)
                {
                    Debug.Log(string.Format("ActionSet {0} is already added.", set.Name), brain);
                    return;
                }
            }
            ...
            ActionSets.Add(set);
            foreach (var action in clonedSet.Actions)
            {
                action.Initialize(brain);
            }
        }

        internal void RemoveRuntimeActionSet(ActionSet set)
        {
            if (!Application.isPlaying) return;
            if (set == null) return;
        ...
        /// <summary>
        /// Deletesthe null values which are created due to renames/deleted classes/...
        /// </summary>
